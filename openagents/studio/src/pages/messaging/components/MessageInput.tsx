...
import { useAutoplayWhenMusicWorkerResultArrives } from "../play_music";
console.log("[view] ThreadMessagingViewEventBased rendered");
interface AgentInfo {
  agent_id: string;
  display_name?: string;
  status?: string;
...
type AudioBase64Attachment = {
  kind: "audio_base64";
  filename: string;
  size: number;
  audio_base64: string; // "data:audio/webm;base64,..."
};

type AttachmentData = CacheAttachment | AudioBase64Attachment;
...
const styles = `
  ...
  background-color: rgba(79, 70, 229, 0.12); /* 浅一点的主色背景 */
  border-color: #6366f1;
  color: #4f46e5;
}
...
const MessageInput: React.FC<MessageInputProps> = ({
  ...
  if (!isRecording) {
    try {
      ...
      if (!recog) {
        console.warn("当前浏览器不支持 Web Speech 语音识别");
      } else {
        try {
          recog.start();
        } catch (e) {
      ...
      console.error("无法获取麦克风权限或启动录音失败:", e);
  ...
  function blobToDataURL(blob: Blob): Promise<string> {
    ...
    reader.onload = () => resolve(String(reader.result)); // "data:audio/webm;base64,..."
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 1) Audio: send as base64 (no cache upload)
    const isAudio =
    ...
    // 2) Non-audio: upload file using HTTP API to shared cache (original logic)
    ...
    console.log("UPLOAD cache_id =", result.cache_id);
        ...
        <div className="input-area">
          ...
          <div className="input-actions">
            ...
            <button
              ...
              disabled={disabled}
              className={`action-button ${currentTheme} ${
                isRecording ? "action-button-mic-active" : ""
              }`}
  ...
  <div className="last-recording">
    ...
    <audio controls src={lastRecordingUrl} style={{ maxWidth: "100%" }} />
