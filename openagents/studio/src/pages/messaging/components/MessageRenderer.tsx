...
function prettifyJianpuForDisplay(jianpu: string): string {
  const DOT_BELOW = "\u0323";
  const REST = "𝄽";
  ...
  const lines: string[] = [];
  const hasZero = tokens.includes("0");
  const perLine = 8;
  ...
  for (const tok of tokens) {
    ...
    const down = (tok.match(/,/g) || []).length;
    let t = tok.replace(/,/g, "");
    t = t.replace(/#/g, "♯").replace(/b/g, "♭");
    if (down > 0) {
      t = t.replace(/[1-7]/, (m) => m + DOT_BELOW.repeat(down));
    }
    current.push(t);

    if (!hasZero && current.length >= perLine) {
      lines.push(current.join(" "));
      current = [];
    }
  }
...
function parsePlayRequestFromText(text: string) {
  if (typeof text !== "string") return null;
  if (text.startsWith("No numbered notation found."))
   return null;



  const m = text.match(/numbered notation[:：]\s*([\s\S]+)$/i);
  //console.log(text)
  const inst = (text.match(/instrument[:：]\s*([a-zA-Z0-9_-]+)/i)?.[1] || "piano").toLowerCase();
  const bpmRaw =
  text.match(/\b(?:bpm|tempo)[:：]\s*`?([0-9]+(?:\.[0-9]+)?)`?/i)?.[1];
  const bpm = bpmRaw ? Math.round(Number(bpmRaw)) : 90;
  const styleRaw =
  text.match(/\b(?:style|beat)[:：]\s*`?(swing|straight)`?\b/i)?.[1];
  const style = (styleRaw || "swing").toLowerCase();


  // note length (beats per token)
  const noteLenRaw =
    text.match(/\b(?:noteLen|note_len|note_len_beats|notelenbeats)[:：]\s*([0-9]+(?:\.[0-9]+)?)/i)?.[1];
  const note_len_beats = noteLenRaw ? Number(noteLenRaw) : 0.4;


  if (m) {
    const jianpu = m[1].trim();
    if (!jianpu) return null;
...
function buildPlayRequestDisplayText(payload: any): string {
  const instrument = (payload?.instrument || "piano").toLowerCase();
  const bpm = payload?.bpm ?? 90;
  const noteLen = payload?.note_len_beats ?? 0.5;


  const style =
    (payload?.style ?? payload?.beat ?? "swing").toLowerCase();

  const jianpu = payload?.jianpu || "";
  const pretty = prettifyJianpuForDisplay(jianpu);

  return [
    `**Audio ready** (instrument: \`${instrument}\`, tempo: \`${bpm}\` BPM, unit: \`${noteLen}\` beat, style: \`${style}\`)`,
    ``,
    `numbered notation:`,
    ``,
    "```",
...
function normalizeJianpu(s: string) {
   return String(s || "")
    .replace(/𝄽/g, " 0 ")
    .replace(/\s+/g, " ")
    .trim();
...
async function playFromPayload(payload: any) {
  const jianpu = normalizeJianpu(payload?.jianpu || "");

  if (!jianpu) return;
  console.log("[playFromPayload] jianpu =", jianpu);
  console.log("[playFromPayload] bpm =", payload?.bpm, "note_len_beats =", payload?.note_len_beats, "instrument =", payload?.instrument, "style =", payload?.style);
  const bpm = Number(payload?.bpm ?? 90);
  const noteLen = Number(payload?.note_len_beats ?? payload?.noteLenBeats ?? 0.5);
  const instrument = String(payload?.instrument ?? "piano");
  console.log("[MessageHeader]", payload?.instrument);
  ...
  await playJianpu(jianpu, bpm, noteLen, instrument,{});
}
  ...
  const renderThreadMessage = (
    ...
    const rawText = messageProps.content || "";


    const playPayload = parsePlayRequestFromText(rawText);
    console.log("[MessageRenderer] playPayload =", playPayload);


    const displayText = playPayload ? buildPlayRequestDisplayText(playPayload) : rawText;
...
//               <MarkdownContent content={messageProps.content} />
          ...
          >
            ...
            {(() => {
          const payload = parsePlayRequestFromText(messageProps.content);
          ...
          return (
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                playFromPayload(payload).catch(console.error);
              }}
              title="Play"
            >
              ▶️
  ...
  const renderUnifiedMessage = (
    ...
    const  isUserMsg =
  message?.senderId === "SwiftMind0783";


    const playPayload = isUserMsg ? null : parsePlayRequestFromText(rawText);
      ...
      <div key={message.id} className="mb-1 relative">
        ...
        >
          ...
          >
            ...
            {(() => {
              const payload = parsePlayRequestFromText(message.content);
              if (!payload) return null;
